...
//	uart_buf_len = sprintf((char *)debugPrintPtr, "DepConv Scratch Buffer size %i\n", buf_size);
//	HAL_UART_Transmit(&huart3, (uint8_t *)(char *)debugPrintPtr, uart_buf_len, 100);
//	debugPrintPtr += 32;

	if (buf_size > INPUT2_TENSOR_BUFFER_1_SIZE) {

		  uart_buf_len = sprintf((char *)debugPrintPtr, "DepConv Scratch Buffer too small for buffer size %u\n", buf_size);
//		  HAL_UART_Transmit(&huart3, (uint8_t *)(char *)debugPrintPtr, uart_buf_len, 100);
		  debugPrintPtr += 32;
