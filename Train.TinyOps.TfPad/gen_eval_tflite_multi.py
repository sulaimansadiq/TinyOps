...
if __name__ == '__main__':
    ...
    def eval_image(data):
        ...
        image = image.permute(0, 2, 3, 1) - 0.5

        quant_scale = input_details[0]['quantization'][0]
        quant_zero_point = input_details[0]['quantization'][0]

        image_np = (image.numpy() / quant_scale + quant_zero_point).astype(np.int8)
        ...
        output_data = np.atleast_2d(output_data)

        pred = np.argmax(output_data, axis=1)
        acc1 = np.sum((target.numpy() == pred).astype(np.float32))

        target5 = np.tile(target.numpy(), (5, 1))
        pred5 = np.argpartition(output_data.T, -5, axis=0)[-5:]
        acc5 = np.sum((pred5 == target5).astype(np.float32))
    ...
    interpreter.resize_tensor_input(input_details[0]['index'],
                                    [args.batch_size, input_shape[1], input_shape[2], input_shape[3]])
