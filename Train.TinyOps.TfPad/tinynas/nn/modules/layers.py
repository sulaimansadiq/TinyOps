...
class My2DLayer(MyModule):
    ...
    def __init__(self, in_channels, out_channels,
                 use_bn=True, act_func='relu', dropout_rate=0, ops_order='pad_weight_bn_act', padding=None, res_in=None):  # pass padding list here
        ...
        """ modules """
        modules = {}
        # if padding passed in, create explicit padding layer in modules before conv
        # put padding into ops_list
        # batch norm

        if self.padding:
            modules['pad'] = nn.ConstantPad2d(self.padding, 0)
        if self.use_bn:
...
class ConvLayer(My2DLayer):
    ...
    def __init__(self, in_channels, out_channels,
                 ...
                 use_bn=True, act_func='relu', dropout_rate=0, ops_order='pad_weight_bn_act', padding=None, res_in=None):
        ...
        super(ConvLayer, self).__init__(in_channels, out_channels, use_bn, act_func, dropout_rate, ops_order, padding)
    ...
    def weight_op(self):
        ...
        if self.padding:    # padding layer explicitly added
            weight_dict['conv'] = nn.Conv2d(
                self.in_channels, self.out_channels, kernel_size=self.kernel_size, stride=self.stride,
                dilation=self.dilation, groups=self.groups, bias=self.bias
            )
        else:               # redundant. all that is left in network is 1x1 conv operations. padding will always be zero
            weight_dict['conv'] = nn.Conv2d(
...
class DepthConvLayer(My2DLayer):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        super(DepthConvLayer, self).__init__(in_channels, out_channels, use_bn, act_func,
                                             dropout_rate, ops_order, padding)
...
class MBInvertedConvLayer(MyModule):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, expand_ratio=6, mid_channels=None,
                 act_func='relu6', use_se=False, padding=None, res_in=None):
        ...
        if self.padding:
            depth_conv_modules = [
                ('pad', nn.ConstantPad2d(self.padding, 0)),
                ('conv', nn.Conv2d(feature_dim, feature_dim, kernel_size, stride, groups=feature_dim, bias=False)),
                ('bn', nn.BatchNorm2d(feature_dim)),
                ('act', build_activation(self.act_func, inplace=True))
            ]
        else:
