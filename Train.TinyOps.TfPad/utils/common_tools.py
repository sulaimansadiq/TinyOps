...
def get_tf_same_padding(kernel_size, res_in, stride):
	if isinstance(kernel_size, tuple):
		assert len(kernel_size) == 2, 'invalid kernel size: %s' % kernel_size
		p1 = get_same_padding(kernel_size[0])
		p2 = get_same_padding(kernel_size[1])
	...
	if stride == 1:									# if stride 1, padding is the same
		padding = [pad, pad, pad, pad]
	elif stride == 2:								# if stride 2 edge case occurs
		pad = kernel_size // 2
		if res_in % 2 == 0:							# edge case where padding differs when stride is 2 and res_in is even
			padding = [pad-1, pad, pad-1, pad]		# TF style padding - left, right, top, bottom, uneven padding added on right and bottom
			# padding = [pad, pad-1, pad, pad-1]	# PyTorch style padding - left, right, top, bottom, uneven padding added on left and top
		else:
			padding = [pad, pad, pad, pad]
