...
class PrefetchedWrapper(object):
    def prefetched_loader(loader):
        ...
        #mean = torch.tensor([0.5, 0.5, 0.5]).cuda().view(1,3,1,1)
        #std = torch.tensor([0.5, 0.5, 0.5]).cuda().view(1,3,1,1)
        mean = torch.tensor([0.5 * 255, 0.5 * 255, 0.5 * 255]).cuda().view(1,3,1,1)
        std = torch.tensor([0.5 * 255, 0.5 * 255, 0.5 * 255]).cuda().view(1,3,1,1)
