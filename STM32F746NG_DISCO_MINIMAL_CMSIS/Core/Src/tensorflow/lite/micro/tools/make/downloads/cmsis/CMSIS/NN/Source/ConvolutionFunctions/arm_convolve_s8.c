...
//extern uint8_t oc_input2_tensors_buffers_1[INPUT2_TENSOR_BUFFER_1_SIZE];
//extern uint8_t oc_input2_tensors_buffers[OP_MAX_INPUT_SIZE][TENSOR_BUFFER_SIZE];
#endif
    ...
    for (i_batch = 0; i_batch < input_batches; i_batch++)
    {
        ...
        for (i_out_y = 0; i_out_y < output_y; i_out_y++)
        {
            for (i_out_x = 0; i_out_x < output_x; i_out_x++)
            {
                ...
                /* Computation is filed for every 2 columns */ /* meaning two receptive fields - receptive field is column */
